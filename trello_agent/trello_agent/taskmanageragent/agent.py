...
def adicionar_tarefa(nome_da_task: str, descricao_da_task: str, due_date: str):
    ...
    meu_board = [b for b in boards if b.name == 'DIO_Agent'][0]
    ...
    minha_lista.add_card(
        ...
        due=corrigir_data(due_date)
    )

def corrigir_data(due_date: str):
    data_str = due_date
    data_obj = datetime.strptime(data_str, '%Y-%m-%d')
    nova_data_obj = data_obj + timedelta(days=1)
    nova_data_str = nova_data_obj.strftime('%Y-%m-%d')
